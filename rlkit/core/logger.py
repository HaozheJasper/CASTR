...
def save_extra_data(data, path='extra_data', ext='.pkl'):
    ...
    """
    os.makedirs(osp.join(_snapshot_dir, 'eval_trajectories'), exist_ok=True)
    file_name = osp.join(_snapshot_dir, path + ext)
    print(f'dumping into {file_name}')
...
def save_itr_params(itr, params_dict):
    """
    snapshot params, default mode='all', save each itr as a different copy
    :param itr:
    :param params_dict: a single ordered dict
    :return:
    """
    # NOTE: assumes dict is ordered, should fix someday
    names = params_dict.keys()
    params = params_dict.values()
    # save params in batch, one file for each network
    if _snapshot_dir:
